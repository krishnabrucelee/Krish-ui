								...
								<table cellspacing="1" cellpadding="1" class="table table-bordered table-striped" id="vpc_ip_address_table">
								    ...
								    <tbody data-ng-show="ipList.length > 0">
								        <tr ng-repeat="ipaddress in ipList">
                      					<td>
                      						<a id="vpc_public_ip_name_button" class="text-info" ui-sref="vpc.view-vpc.config-vpc.public-ip.ip-view({id1:ipaddress.id})"  title="View IP"> {{ ipaddress.publicIpAddress }} <span ng-if="ipaddress.isSourcenat">[Source NAT]</span></a>
                      					</td>
                      					<td>{{ipaddress.zone.name}} </td>
                      					<td>{{ipaddress.network.name}}</td>
                      					<td> <b class="text-success text-uppercase">{{ipaddress.state}}</b></td>
                      					<td>
                          					<a id="vpc_enable_vpc_button" data-ng-if="ipaddress.isSourcenat && ipaddress.vpnState != 'RUNNING'" class="icon-button" title="Enable VPN" data-ng-click="enableVpn('md',ipaddress)"><i class="custom-link-icon custom-icon-ip-disabled"></i></a>
                          					<a id="vpc_disable_vpc_button" data-ng-if="ipaddress.isSourcenat && ipaddress.vpnState == 'RUNNING'" class="icon-button" title="Disable VPN" data-ng-click="disableVpn('sm',ipaddress)"><i class="custom-link-icon custom-icon-ip"></i></a>
                         					<a id="vpc_release_ip_button" data-ng-if="!ipaddress.isSourcenat" class="icon-button" title="Release IP" data-ng-click="releaseIP('sm',ipaddress)"><span class="fa fa-chain-broken"></span></a>
                      					</td>
              							</tr>
